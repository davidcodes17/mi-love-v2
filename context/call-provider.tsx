...
const userTokens: Record<string, string> = {
  user1: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoidXNlcjEifQ.W4x-FSYSxEpzvV4s2lCrWp8O3YmYMSOGLaXZwAzp26s",
  user2: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoidXNlcjIifQ.1j31hu5jOeclEczJxDBJ2rlr-s7Wac9FtiwMsGd-cOw"
};
...
export const CallProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log("Initializing client for:", user.id, "with token available:", !!token);
      ...
      const connectionChangedHandler = (event) => {
...
const InternalStreamCall = ({ children }: { children: ReactNode }) => {
  ...
  return calls.length > 0 ? <StreamCall call={calls[0]}>{children}</StreamCall> : <>{children}</>;
};
